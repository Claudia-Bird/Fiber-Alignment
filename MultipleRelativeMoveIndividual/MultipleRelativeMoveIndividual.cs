            ...
            else
            {
                ...
                if (cmdLib.Open(strDeviceKey))
                {
                    ...
                    for (int nMotor = 1; nMotor <= nMotorMax; nMotor++)
                    {
                        ...
                        bStatus = cmdLib.SetZeroPosition(strDeviceKey, nMotor);
                        ...
                        if (bStatus)
                        {
                            Console.WriteLine("Motor #{0}', Start Position = {1}", nMotor, nPosition);
                            cmdLib.WriteLog("Motor #{0}', Start Position = {1}", nMotor, nPosition);
                        }
                        else
                        {
                            Console.WriteLine("Motor #{0}', GetPosition Error.", nPosition);
                            Console.WriteLine("Motor #{0}', GetPosition Error.", nPosition);
                        }
                        ...
                        Console.WriteLine("Motor#{0} told to move {1}", nMotor, nSteps);
                    }
                }
                // Close the device
